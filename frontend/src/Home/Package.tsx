...
const PackageSection = () => {
  ...
  const categories = useMemo(() => {
    ...
    const merged = baseCats.concat(
      displayCats.filter((c) => !baseCats.includes(c) && c.toLowerCase() !== 'premium')
    );
  ...
  // reset slide when filters change (prevents transient out-of-range)
  useEffect(() => {
    setCurrentSlide(0);
  ...
  const getVisibleCards = () => {
    ...
    if (isMobile) {
      const idx = safeMod(currentSlide, totalSlides); // wrap on mobile too
      return [filteredPackages[idx]];
    }
  ...
  if (loading) return <div className="py-12 text-center text-black">Loading packages...</div>;
  if (error) return <div className="py-12 text-center text-red-500">{String(error)}</div>;
          ...
          <div className="relative">
            ...
            <div className={isMobile ? 'mx-2' : 'mx-8'}>
              <div className={`grid ${isMobile ? 'grid-cols-1' : 'lg:grid-cols-2'} gap-6`}>
                {getVisibleCards()
                  .filter(Boolean) // defensive: no undefined cards
                  .map((pkg) => (
                    <div key={pkg!.id}>
                      {isMobile ? (
                        // Mobile card
