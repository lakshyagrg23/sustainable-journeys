...
function PackagePage() {
  ...
  const paramValue: unknown = (params as Record<string, unknown>)?.slug;
  const rawId = Array.isArray(paramValue) ? paramValue[0] : paramValue;
  const { singlePackage, fetchPackageBySlug, loading, error } = useTravelStore();
  ...
  useEffect(() => {
    if (rawId) {
        fetchPackageBySlug(rawId as string);
    }
  }, [rawId, fetchPackageBySlug]);
  ...
  useEffect(() => {
    ...
    if (title) {
      document.title = `${title} - Sustainable Journeys`;
    }
  ...
  if (!rawId) {
  return <div className="p-8 text-center text-red-500">No slug provided in route.</div>;
}
            ...
            <div className="text-white max-w-lg rounded-xl p-8" >
              ...
              <div className="flex flex-wrap gap-4 items-center mb-4">
                ...
                <div className="flex items-center bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full">
                  ...
                  <span>{(singlePackage as any)?.region?.name || "Nepal"}</span>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="flex flex-wrap gap-2 items-center mb-2">
                ...
                <div className="flex items-center bg-white/20 backdrop-blur-sm px-2 py-1 rounded-full">
                  ...
                  <span className="text-xs">{(singlePackage as any)?.region?.name || "Nepal"}</span>
                </div>
                <div className="flex items-center bg-white/20 backdrop-blur-sm px-2 py-1 rounded-full">
                  <span className="flex mr-1">{renderStars(rating)}</span>
